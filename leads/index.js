      ...
      if (action === "setup" && method === "GET") result = await service.migration(user, false);
        ...
        if (customerWrite) {
          ...
          result = Object.prototype.hasOwnProperty.call(body, "name")
            ? await legacy.enqueueDbMutation(() => service.updateResource(user, req.headers["x-idempotency-key"], leadId, body))
            : await service.updateTag(user, req.headers["x-idempotency-key"], leadId, body);
        }
