...
const mineList = vm.runInContext("renderLeadMineList([{id:'mine-1',name:'王师傅',phone:'13800000001',tags:'工人',wechatStatus:'agreed_pending',lastFollowupAt:'2026-09-14T00:00:00Z',lastFollowupContent:'确认明天下午送样',followupCount:3,createdAt:'2026-09-01T00:00:00Z'}])", context);
assert(mineList.includes("lead-list-mine") && mineList.includes("确认明天下午送样") && mineList.includes("3 次") && mineList.includes("同意但未通过") && mineList.includes("leadWechatStatusSave"));
